...
| Parameter                                | Description                                                           | Default            |
| ---------------------------------------- | --------------------------------------------------------------------- | ------------------ |
| `ldap.enabled`                  | Enable internal OpenLDAP server                                       | `false`            |
| `ldap.image.registry`           | OpenLDAP image registry                                               | `docker.io`        |
| `ldap.image.repository`         | OpenLDAP image repository                                             | `bitnami/openldap` |
| `ldap.image.tag`                | OpenLDAP image tag                                                    | `"2.6"`            |
| `ldap.image.pullPolicy`         | Image pull policy                                                     | `IfNotPresent`     |
| `ldap.existingSecret`           | Name of existing Kubernetes Secret (must contain key `adminPassword`) | `""`               |
| `ldap.adminPassword`            | Admin password (ignored if `existingSecret` is set)                   | `adminpass`        |
| `ldap.resources`                | CPU/Memory resource requests/limits                                   | See values.yaml    |
| `ldap.persistence.enabled`      | Enable persistence for OpenLDAP                                       | `true`             |
| `ldap.persistence.size`         | Size of the persistent volume                                         | `1Gi`              |
| `ldap.persistence.storageClass` | Storage class                                                         | `""`               |
| `ldap.persistence.accessMode`   | Access mode                                                           | `ReadWriteOnce`    |

> 💡 If `ldap.existingSecret` is set, it must contain a key named `adminPassword`.
